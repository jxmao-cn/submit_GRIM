...
class SpGraphAttentionLayerV2(nn.Module):
    ...
    - Dynamic attention (strict concatenation): e_ij = a^T LeakyReLU(W_pair · [h_i || h_j])
      (replaces static a^T [W h_i || W h_j] concatenation).
    ...
    def forward(self, input, adj):
        ...
        # Numerical stabilization: clip logits to avoid exp overflow
        edge_e_raw = torch.clamp(edge_e_raw, min=-20.0, max=20.0)
        ...
        e_rowsum = self.special_spmm(edge, edge_e, torch.Size([N, N]), torch.ones(size=(N,1), device=input.device))
        # Prevent division by zero in isolated cases
        e_rowsum = e_rowsum.clamp_min(1e-12)
        # e_rowsum: N x 1
...
class SpGATv2(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads):
        ...
        self.out_att = SpGraphAttentionLayerV2(nhid * nheads,
                                               ...
                                               alpha=alpha,
                                               concat=False)
        # Graph-level readout: pool node features to a single graph embedding, then map to scalar
    ...
    def forward(self, x, adj):
        ...
        x = F.elu(torch.cat([att(x, adj) for att in self.attentions], dim=1))
        x = F.dropout(x, self.dropout, training=self.training)
        x = F.elu(self.out_att(x, adj))  # node-level features: [N, nclass]
        graph_emb = x.mean(dim=0, keepdim=True)  # [1, nclass]
        out = self.fc_out(graph_emb)  # [1, 1]
        return out
